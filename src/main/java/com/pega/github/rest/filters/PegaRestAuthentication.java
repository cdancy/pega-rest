...
package com.pega.github.rest.filters;
...
@Singleton
public class PegaRestAuthentication implements HttpRequestFilter {
    private final Supplier<Credentials> creds;

    public static final String BASE64_REGEX = "^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)$";

    @Inject
    PegaRestAuthentication(@Provider Supplier<Credentials> creds) {
        this.creds = creds;
    }
            ...
            */
            ...
            if (isBase64Encoded(foundCredential)) {
                return request.toBuilder()
                        .addHeader(HttpHeaders.AUTHORIZATION, "Basic " + foundCredential).build();
            } else {
